...
export default function PropertyCard({ property }: { property: Property }) {
  return (
    <article className={`bg-white rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 group cursor-pointer h-full flex flex-col ${property.className || ''}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {property.type && (
          <div className={`absolute bottom-3 left-3 ${property.type === 'FOR RENT' ? 'bg-mosque/90' : 'bg-nordic-dark/90'} text-white text-xs font-bold px-2 py-1 rounded`}>
            {property.type}
          </div>
        )}
      </div>
